				  ...
				  $.ajax({
					  ...
					  success:function(msj){
						  if ( msj == 1 ){
							  ...
							  aparecermodulos(archivo_vizualizar+"?idmenumodulo="+idmodulo+"&ac=1",donde_mostrar);
						  }
						  else{
							  overlayclose('abc');
							   swal("Error al Borrar", {
							    });
							  aparecermodulos(archivo_vizualizar+"?idmenumodulo="+idmodulo+"&ac=0"+msj,donde_mostrar);
						  }							  
					  },
					  error:function(){
					  	swal("Error al Borrar", {
...
function buscarclientes(formulario)
{
	//alert ("entro a buscarPedido");
	console.log("entro a buscarPedido");
	var cadena = ObtenerDatosFormulario(formulario);
	...
	$.ajax({
			  type: 'POST',
			  url: 'catalogos/pacientes/li_pacientes.php',
			  data: cadena,
			  cache:false,
			  success:function(msj){
