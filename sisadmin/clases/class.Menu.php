...
{   
	...
	WHERE modulos_menu.estatus=1 AND perfiles_permisos.idperfiles=$this->idperfil AND modulos_menu.idmodulos=".$rows['idmodulos']." ORDER BY nivel";
				...
				if($totalRow>0)
				{
					$menu.='<li class="nav-item"><a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapse'.$rows['idmodulos'].'" aria-expanded="true" aria-controls="collapse'.$rows['idmodulos'].'"><i class="'.$rows['icono'].'"></i><span>'.$rows['modulo'].'</span></a>';
					$menu.='<div id="collapse'.$rows['idmodulos'].'" class="collapse" aria-labelledby="headingUtilities" data-parent="#accordionSidebar"><div class="bg-white py-2 collapse-inner rounded"> <h6 class="collapse-header">'.$rows['modulo'].':</h6>';
					...
					do
					{
						...
						$menu.='<a class="collapse-item" onClick="aparecermodulos(\''.$row['ubicacion_archivo'].$row['archivo'].'?idmenumodulo='.$row['idmodulos_menu'].'\',\'main\'); return false;"><i class="'.$row['icono'].'"></i>  '. $row['menu'].'</a>';
